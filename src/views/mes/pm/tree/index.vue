<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="code" label="项目编码" min-width="180"></el-table-column>
      <el-table-column prop="type" label="类型" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.mes_project_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
        ...
        <el-row>
          <el-col :span="24" v-if="form.parentId !== 0">
            <el-form-item label="上级项目" prop="parentId">
              <treeselect v-model="form.parentId" :options="treeOptions" :normalizer="normalizer" placeholder="选择上级项目" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="项目类型" prop="type">
              <el-select v-model="form.type" placeholder="请选择项目类型">
                <el-option
                  v-for="dict in dict.type.mes_project_type"
                  :key="dict.value"
                  :label="dict.label"
                  :value="dict.value"
                ></el-option>
...
<script>
import { listTree, getTree, delTree, addTree, updateTree, listTreeExcludeChild } from "@/api/mes/pm/tree";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";

export default {
  name: "Dept",
  dicts: ['sys_yes_no', 'mes_project_type'],
  ...
  methods: {
    ...
    getList() {
      ...
      listTree(this.queryParams).then(response => {
        debugger;
        var trees = this.handleTree(response.data, "id");
        this.treeList = trees;
        this.loading = false;
      });
    },
    ...
    handleAdd(row) {
      ...
      listTree().then(response => {
        this.treeOptions = this.handleTree(response.data, "id");
      });
    },
    /** 展开/折叠操作 */
    toggleExpandAll() {
    ...
    handleUpdate(row) {
      ...
      listTreeExcludeChild(row.id).then(response => {
        this.treeOptions = this.handleTree(response.data, "id");
      });
    },
    /** 提交按钮 */
